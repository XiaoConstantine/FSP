...
class TableCanvas(Canvas):
    ...
    def __init__(self, parent=None, model=None, newdict=None, width=None, height=None, **kwargs):
        ...
        self.currentcol = 0
        self.reverseorder = 0
        self.upperColumnHeaders=kwargs['upperColumnHeaders']
        ...
        self.tablecolheader = ColumnHeader(self.parentframe, self,self.upperColumnHeaders)
    ...
    def createfromDict(self, data,columns=None):
...
#            namefield=columns[0]
        ...
        self.model.importDict(data, None,columns)
...
class ColumnHeader(Canvas):
    ...
    def __init__(self, parent=None, table=None,upperColumnHeaders=None):
        self.height = 20
        
        self.upperColumnHeaders = upperColumnHeaders
        if upperColumnHeaders !=None:
                self.height *=2
        Canvas.__init__(self, parent, bg='gray25', width=500, height=self.height )
    ...
    def redraw(self):
        ...
        current_index = 0
        if self.upperColumnHeaders !=None:
            for upperColumnHeader in self.upperColumnHeaders:
                colspan = upperColumnHeader['colspan']
                text = upperColumnHeader['text']
                w = 0
                for col in range(current_index,current_index+colspan):
                    colname=self.model.columnNames[col]
                    if self.model.columnwidths.has_key(colname):
                        w1=self.model.columnwidths[colname]
                    else:
                        w1=self.table.cellwidth
                    w+=w1
                x=self.table.col_positions[current_index]
                self.create_line(x, 0,x, h/2, tag=('gridline', 'vertline'),
                                 fill='white', width=2)
                self.create_text(x+w/2,h/4,
                                text=text,
                                fill='white',
                                font=self.thefont,
                                tag='text')
                self.create_line(x,h/2, x+w,h/2, tag='gridline',
                        fill='white', width=2)
                ...
                for col in range(cols):
                    ...
                    line = self.create_line(x, h/2, x, h, tag=('gridline', 'vertline'),
                                 fill='white', width=2)

                    self.create_text(x+w/2,h*3/4,
                                text=collabel,
                                fill='white',
                                font=self.thefont,
                                tag='text')
                    x=self.table.col_positions[col+1]
                    self.create_line(x,h, x+w,h, tag='gridline',
                        fill='white', width=2)
        else:
                for col in range(cols):
                    colname=self.model.columnNames[col]
