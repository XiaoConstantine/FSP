...
int main()
{
	...
	if( gethostname(hostname,sizeof(hostname)) ){
		fprintf(stderr," XC gethostname(...) failed errno: %d\n", errno);
		exit(1);
	}
	...
	if ((hp = gethostbyname(hostname)) == 0) {
		fprintf(stderr,"XC gethostbyname(...) failed errno: %d exiting...\n", errno);
		exit(1);
	}
...
//		ntohs(sock_addr_from_client.sin_port));
	...
	int ret = 0;
        char	msg[MSGSIZE];
	fprintf(stderr," XC: sizeof msg: %d\n", sizeof(msg));
	...
	while (ret = recv(sd_current, msg, sizeof(msg), 0) > 0) {
		...
		if (send(sd_current, msg, strlen(msg), 0) == -1) {
			fprintf(stderr,"XC send(...) failed errno: %d exiting...\n", errno);
			exit(1);
		}
	}

	if( 0 == ret ) {
		printf("XC ciient exited...\n");
		/* For TCP sockets	
